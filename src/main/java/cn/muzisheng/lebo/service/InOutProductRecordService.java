...
public interface InOutProductRecordService {
    /**
     * 获取所有商品出入库记录列表，筛选条件有时间区间，商品名称，出入库类型, 商品ID
     * @param InOutProductRecordSelectDTO 筛选条件
     * @return 商品入库记录列表
     */
    ResponseEntity<List<InOutProductRecord>> getInOutRecordList(InOutProductRecordSelectDTO InOutProductRecordSelectDTO);
